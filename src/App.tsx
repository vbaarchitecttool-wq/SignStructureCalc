...
const G = 9.8; // m/s^2
const windPressure = (V) => 0.613 * V * V; // N/m²
const sigmaAllow = (Fy) => (2 / 3) * Fy; // N/mm²（一時設計）
const toKgf = (N) => N / G;
const fmt = (n, unit = "", digits = 2) => {
  const v = Number(n);
...
function computePlateThickness(T_row, a_mm, s_mm, plateFy) {
...
/** @typedef {{name:string,d:number,Ta:number,Va:number,min_e?:number,min_s?:number,hefRec?:number}} AnchorSpec */
/** @typedef {{family:string,name:string,A_cm2?:number,w_kgpm?:number,Ix_cm4?:number,Iy_cm4?:number,ix_cm?:number,iy_cm?:number,Zx_cm3?:number,Zy_cm3?:number}} SectionSpec */
...
export default function App() {
  ...
  const [plateD, setPlateD] = useState(0.4); // m（ベースプレート奥行：未入力だったため追加）
  const [holeClearance, setHoleClearance] = useState(2); // mm（孔径= d + clearance のルール）
  const [finishSpec, setFinishSpec] = useState("未定"); // 例：溶融亜鉛、塗装など
  const [concSpec, setConcSpec] = useState(
    "Fc=入力値、スランプ/養生は現場仕様"
  ); // テキスト
  const [siteNotes, setSiteNotes] = useState(""); // 特記事項
...
const [hasInterPostConnection, setHasInterPostConnection] = useState(true);
  ...
  const [ecc, setEcc] = useState(0.05); // m (今後拡張用)
  ...
  const [embedDepth, setEmbedDepth] = useState(0.6); // m（根入れ深さ：未使用）
  ...
  const [a_clear, setAclear] = useState(80); // mm: 柱縁〜ボルト心の片持ち長
  const [plateT, setPlateT] = useState(16); // 採用板厚 mm（選択肢内）

  // ===== 荷重 =====
  const [windV, setWindV] = useState(34);
  ...
  // 看板全体
  const Fw_total = q * shapeCf * panelArea; // N
...
const nCol = hasInterPostConnection ? nCol_raw : 1;
  ...
  const V_anchor = Fh / Math.max(1, anchorQty);
  ...
  // アンカー：コンクリート側引張耐力（簡易）
  const Ta_conc = 0.25 * Fc * d * anchorEmbed; // N
  ...
  function evalFoundation(B: number, D: number, H: number) {
    const N = concUnitW * 1000 * (B * D * H) + Wself_perCol; // N
    const A = B * D; // m²

    const e = M / Math.max(N, 1); // m
    const e_lim = B / 6; // m
    const sigma_avg = N / Math.max(A, 1e-9); // N/m²
    ...
    if (e <= e_lim) {
      sigma_max = sigma_avg * (1 + (6 * e) / B);
      sigma_min = sigma_avg * (1 - (6 * e) / B);
      noUplift = sigma_min >= 0;
    } else {
      const x = Math.max(B / 2 - e, 0);
      const B_eff = Math.max(3 * x, 0);
      const A_eff = Math.max(B_eff * D, 1e-6);
      sigma_max = (2 * N) / A_eff;
      sigma_min = 0;
      noUplift = false;
    }
    ...
    const qa_allow_conc = 0.25 * Fc * 1000; // kPa
    const qa_allow_final = Math.min(qa_allow_soil, qa_allow_conc);
    const bearingOK = sigma_max / 1000 <= qa_allow_final;

    // ===== 転倒：浮上り時に過大表示しないFS_OT（B/3）=====
    const leverArm_OT = noUplift ? B / 2 : B / 3;
    const FS_OT = (N * leverArm_OT) / Math.max(M, 1e-9);

    const OT_OK = allowUpliftOK
      ? FS_OT >= reqFS_OT
      : FS_OT >= reqFS_OT && noUplift;

    const N_eff = noUplift ? N : N * 0.5;
    const FS_SL = (mu * N_eff) / Math.max(Fh, 1e-9);
  ...
  // ===== JSON IO (DB) =====
  function handleImportAnchors(file) {
  ...
  function handleImportSections(file) {
    const r = new FileReader();
    r.onload = () => {
      try {
        const json = JSON.parse(String(r.result));
  ...
  // ===== 設定入出力 =====
  const hiddenCfgInputRef = useRef(null);
  function buildConfig(allowUpliftOK, foundationOptMode) {
    return {
      __app: "SignWizard",
      __ver: "B-Fc36-ABR-20D-PLT-SELECT",
      forceUnit,
      pdfSigmaDigits,
      pdfMDigits,
      pdfRoundingMode,
  ...
  function applyConfig(cfg) {
    ...
    if (num(cfg.windV)) setWindV(cfg.windV);
    ...
    else if (num(cfg.memberCount))
      setPostQty(Math.max(1, Math.floor(cfg.memberCount)));
    ...
    if (str(cfg.anchorName)) {
      ...
      const pick = list.find((a) => a.name === cfg.anchorName) || list[0];
      if (pick) {
        setAnchor(pick);
        const minHef =
          signTypeFromCfg === "freestanding"
  ...
  h1{font-size:14px;margin:0 0 4px}     /* 18 → 14 */
  h2{font-size:11px;margin:6px 0 3px}   /* 14 → 11, 10px → 6px */
  table{border-collapse:collapse;width:100%;font-size:9.5px} /* 11 → 9.5 */
  th,td{
    border:1px solid #ddd;
    padding:2px 4px;           /* 4x6 → 2x4 */
    text-align:left;
    vertical-align:top;
  }
  ...
  <th>柱間連結</th>
  <td colspan='5'>
    ${hasInterPostConnection ? "あり（均等分担前提）" : "なし（計算は安全側に1本柱扱い）"}
  </td>
</tr>
        <tr>
          <th>水平力 Fh（1本あたり）</th>
        ...
        <tr>
          ...
          <th>基準</th>
          <td>
            OT≥${reqFS_OT}, SL≥${reqFS_SL},
            qa,conc=${qa_allow_final.toFixed(0)} kPa
          </td>
        </tr>
      </table>
      ...
      <h2>3) 基礎（1本あたり：転倒・滑動・地盤＋コンクリート）</h2>
      <table>
        <tr>
          <th>N(基礎＋看板自重/本)</th><td>${showF(
            N_foundation,
            forceUnit
          )}</td>
          <th>偏心 e / B</th>
      ...
      <table>
        <tr>
          <th>仕上げ</th><td colspan="5">${escapeHtml(finishSpec) || "—"}</td>
        </tr>
        <tr>
          <th>孔径ルール</th>
          <td colspan="5">
      ...
      anchor?.d ? (anchor.d + Number(holeClearance)).toFixed(0) : "—"
        ...
        <tr>
          <th>コンクリート仕様</th><td colspan="5">${
            escapeHtml(concSpec) || "—"
          }</td>
        </tr>
        <tr>
          <th>適用基準・条文</th><td colspan="5">${
            escapeHtml(lawRef) || "—"
          }</td>
        </tr>
        <tr>
          <th>特記事項</th>
          <td colspan="5">${escapeHtml(siteNotes) || "—"}</td>
        </tr>
      </table>
<!-- ===== 施工・製作チェックリスト ===== -->
<h3>施工・製作チェック項目</h3>
<table>
  <tr>
    ...
    <th style="width:64%">確認項目</th>
    <th style="width:15%">施工</th>
    <th style="width:15%">確認</th>
  </tr>
  <tr>
    <td>1</td>
    <td>基礎寸法（B×D×H）が本図・本指示書と一致している</td>
    <td>□</td><td>□</td>
  </tr>
  <tr>
    <td>2</td>
    <td>アンカー径・本数・配置・埋込み長さが一致している</td>
    <td>□</td><td>□</td>
  </tr>
  <tr>
    <td>3</td>
    <td>アンカー孔径（d＋クリアランス）が守られている</td>
    <td>□</td><td>□</td>
  </tr>
  <tr>
    <td>4</td>
    <td>ベースプレート板厚・材質・仕上げが一致している</td>
    <td>□</td><td>□</td>
  </tr>
  <tr>
    <td>5</td>
    <td>現地条件（地盤・突出・荷重条件）に変更が無い</td>
    <td>□</td><td>□</td>
  </tr>
</table>
...
<div class="muted">
  <ul>
    ...
    <li>浮上り許容モードを使用している場合、施工誤差・偏心増大に十分留意すること。</li>
  </ul>
</div>
...
if (signType === "freestanding") {
  ...
  // 初期スコアも既定値から算出（allowUpliftOK の現状態で評価）
  let bestScore =
    foundationOptMode === "VOL"
      ? DEFAULT_FOOT.B * DEFAULT_FOOT.D * DEFAULT_FOOT.H
      : DEFAULT_FOOT.B * 100 + DEFAULT_FOOT.D * 10 + DEFAULT_FOOT.H;

  // 既定値がそもそもOKなら「found=true」でスタート（比較の基準にする）
  {
    const r0 = evalFoundation(DEFAULT_FOOT.B, DEFAULT_FOOT.D, DEFAULT_FOOT.H);
    if (r0.ok) found = true;
  }
  ...
  for (let B = 0.6; B <= 2.0; B += 0.05) {
    for (let D = 0.6; D <= 2.0; D += 0.05) {
      for (let H = 0.4; H <= 2.0; H += 0.05) {
        const r = evalFoundation(B, D, H); // ★ここで allowUpliftOK 現在値が効く
        if (!r.ok) continue;
...
// ★OKが見つかったら採用、見つからなくても「最良の近傍案」を提示する
if (found) {
  setFootB(Number(bestB.toFixed(2)));
  setFootD(Number(bestD.toFixed(2)));
  setFootH(Number(bestH.toFixed(2)));
} else {
  // ---- 近傍案探索：OK未達でも「達成度」が最大の案を探す ----
  let bestNear = null; // {B,D,H, r, scoreNear}
  for (let B = 0.6; B <= 2.0; B += 0.05) {
    for (let D = 0.6; D <= 2.0; D += 0.05) {
      for (let H = 0.4; H <= 2.0; H += 0.05) {
        ...
        const rateBE = (r.qa_allow_final * 1000) / Math.max(r.sigma_max, 1e-9); // (許容/実) ※sigma_maxはN/m2

        // 総合達成度：最も弱い項目を最大化（ボトルネック改善）
        const rateMin = Math.min(rateOT, rateSL, rateBE);

        // 体積 or BD優先は、同程度の達成度なら小さい方を優先
        const sizePenalty =
          foundationOptMode === "VOL" ? (B * D * H) : (B * 100 + D * 10 + H);

        // 近傍スコア：達成度優先（大きいほど良い）→ penaltyは小さいほど良い
        const scoreNear = rateMin * 100000 - sizePenalty; // スケールは適当でOK
  ...
  if (bestNear) {
    ...
    alert(
      "AI基礎最適化：OK解が見つかりませんでした。\n" +
        `ただし、最も近い案へ更新しました（allowUpliftOK=${allowUpliftOK ? "ON" : "OFF"}）。\n` +
        `B=${bestNear.B.toFixed(2)} D=${bestNear.D.toFixed(2)} H=${bestNear.H.toFixed(2)}\n` +
        `達成度(min)=${Math.min(
          bestNear.r.FS_OT / Math.max(reqFS_OT, 1e-9),
          bestNear.r.FS_SL / Math.max(reqFS_SL, 1e-9),
          (bestNear.r.qa_allow_final * 1000) / Math.max(bestNear.r.sigma_max, 1e-9)
        ).toFixed(3)}\n` +
        "不足項目（転倒/滑動/支持力）を結果欄で確認してください。"
    );
  }
}
  // found=false の場合は何もしない（②で改善案を提示します）
  if (found) {
    setFootB(Number(bestB.toFixed(2)));
    setFootD(Number(bestD.toFixed(2)));
    setFootH(Number(bestH.toFixed(2)));
      ...
      >
        ...
        <div style={{ fontSize: 12 }}>
          ...
          <select
            style={{
              width: "auto",
              maxWidth: 250, // ← ここで横幅を制御
            }}
            value={foundationOptMode}
            onChange={(e) => setFoundationOptMode(e.target.value)}
          >
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        >
          ...
          >
            ...
            >
              <span>
  柱本数={nCol_raw}本（計算扱い={nCol}本{hasInterPostConnection ? "" : "：連結なし"}）
</span>
...
>
  ...
  <input
    ...
    value={postQty}
    onChange={(e) =>
      setPostQty(Math.max(1, Math.floor(Number(e.target.value))))
    }
    style={{
      width: "100%",
      borderRadius: 4,
  ...
  >
    <input
      ...
      onChange={(e) => setHasInterPostConnection(e.target.checked)}
    />
    <span>柱間連結あり（耐風梁・ブレース等で荷重が均等分担される）</span>
  </label>
      ...
      本計算は安全側に「1本柱扱い」で検定しています（耐風梁・ブレース等の検討推奨）。
            ...
            >
              ...
              <NumInput
                ...
                step={0.1}
              />
              <ReadOnly label="風圧 q" value={fmt(q, " N/m²")} />
            </div>
            ...
            {signType === "freestanding" && (
              <>
                ...
                <PassRow
                  ...
                  tip={
                    allowUpliftOK
                      ? "浮上り有でもFS満足ならOK（ただし設計上の取扱いは要注意）"
                      : "目安≥1.5（FSは満足でも浮上り有で注意ありになる）"
                  }
                />
        ...
        >
          ...
          <div>
            本ツールは一次設計の簡易法（Σy²配分・片持ち板曲げ・偏心圧＋滑動・簡易アンカーコンクリ判定）を実装したものです。
...
function NumInput({ label, unit, value, onChange, min = 0, step = 0.1 }) {
...
function PassRow({ label, value, ok, tip }) {
  return (
    <div
      style={{
        marginBottom: 4,
